...
function paginate_links( $args = '' ) {
	...
	if ( $args['prev_next'] && $current && 1 < $current ) :
		 ...
		 */
		$page_links[] = '<li><a class="prev page-numbers" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '"><button type="button" class="prev-btn">' . $args['prev_text'] . '</button></a></li>';
	endif;
	for ( $n = 1; $n <= $total; $n++ ) :
		if ( $n == $current ) :
			$page_links[] = "<li class='active'><button type='button' class='news-btn'>" . $args['before_page_number'] . number_format_i18n( $n ) . $args['after_page_number'] . "</button></li>";
			$dots = true;
		else :
			if ( $args['show_all'] || ( $n <= $end_size || ( $current && $n >= $current - $mid_size && $n <= $current + $mid_size ) || $n > $total - $end_size ) ) :
				...
				/** This filter is documented in wp-includes/general-template.php */
				$page_links[] = "<li><a class='page-numbers' href='" . esc_url( apply_filters( 'paginate_links', $link ) ) . "'><button type='button' class='news-btn'>" . $args['before_page_number'] . number_format_i18n( $n ) . $args['after_page_number'] . "</button></a></li>";
				$dots = true;
	...
	if ( $args['prev_next'] && $current && ( $current < $total || -1 == $total ) ) :
		...
		/** This filter is documented in wp-includes/general-template.php */
		$page_links[] = '<li><a class="next page-numbers" href="' . esc_url( apply_filters( 'paginate_links', $link ) ) . '"><button type="button" class="next-btn">' . $args['next_text'] . '</button></a></li>';
	endif;
	switch ( $args['type'] ) {
		case 'array' :
			return $page_links;
